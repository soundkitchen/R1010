...
- `scsynth` 起動時は CoreAudio から既定出力デバイスの nominal sample rate を取得し、`-S` で明示する
...
- sample rate を解決できた場合は `-S <sample-rate>` を優先して付与し、再起動後の CoreAudio 状態差分で pitch / tempo がずれる余地を減らす
- `-S` 付きプロファイルで起動できない場合のみ、従来の sample rate 未指定プロファイルへフォールバックする
